document.addEventListener('DOMContentLoaded', function(){
  // 新規投稿・編集ページのフォームを取得
  const trainingMenuForm = document.getElementById('new_training_menu');
   // プレビューを表示するためのスペースを取得
   const previewList = document.getElementById('previews');
  // 新規投稿・編集ページのフォームがないならここで終了。「!」は論理否定演算子。
  if (!trainingMenuForm) return null;
  ...
  const buildNewFileField = () => {
    ...
    newFileField.setAttribute('name', 'training_menu[images][]');

    // 最後のfile_fieldを取得
    const lastFileField = document.querySelector('input[type="file"][name="training_menu[images][]"]:last-child');
  ...
  const fileField = document.querySelector('input[type="file"][name="training_menu[images][]"]');
